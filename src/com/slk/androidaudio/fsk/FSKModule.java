...
public class FSKModule {
	...
	private static int MINUMUM_NPEAKS = 100; // if lower it means that there is
												// no signal/message
	private static int MAXIMUM_NPEAKS = 110; // if lower it means that there is
	// no signal/message
	...
	private int[] get8Bits(int number) {
		int[] bits = new int[8];
		String msg = "[Message : " + number + "] [Number : "
				+ ErrorDetection.decodeMessage(number) + "] [Bits : ";
		for (int i = 0; i < 8; i++) {
	...
	public static boolean signalAvailable(double[] sound) {
		...
		do {
			...
			i++;
			startIndex = endIndex;
			if (nPeaks > MINUMUM_NPEAKS && nPeaks < MAXIMUM_NPEAKS) {
				Log.i("TAG2", "signalAvailable() nPeaks=" + nPeaks);
				return true;
			}
		} while (i < nParts);
		if (nPeaks > 3)
